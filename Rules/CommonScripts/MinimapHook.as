...
void CalculateMinimapColour( CMap@ map, u32 offset, TileType tile, SColor &out col)
{
	...
	if (map.isTileGold(tile) || tile >= CMap::tile_gold_fake && tile < CMap::tile_gold_fake + 16)
	{ 
		col = show_gold ? color_gold : color_dirt;
	} 
	else if (map.isTileGround(tile) || tile >= CMap::tile_ground_fake && tile < CMap::tile_ground_fake + 16)
	{
		col = color_dirt;
	}
	else if (map.isTileThickStone(tile) || tile >= CMap::tile_thickstone_fake && tile < CMap::tile_thickstone_fake + 16)
	{
		col = color_thickstone;
	}
	else if (map.isTileStone(tile) || tile >= CMap::tile_stone_fake && tile < CMap::tile_stone_fake + 16)
	{
		col = color_stone;
	}
	else if (map.isTileBedrock(tile) || tile >= CMap::tile_bedrock_fake && tile < CMap::tile_bedrock_fake + 16)
	{
		col = color_bedrock;
	}
