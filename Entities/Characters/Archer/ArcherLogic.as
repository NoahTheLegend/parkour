	...
	if (archer.grappling)
	{
		...
		else
		{
			...
			else if (archer.grapple_id == 0xffff) //not stuck
			{
				...
				bool cancel_grapple = false;
				Tile tile = map.getTile(next);

				f32 grapvel = archer.grapple_vel.Length();
				if (grapvel >= 8.0f)
				{
					for (u8 i = 0; i < Maths::Floor(grapvel/8); i++)
					{
						Vec2f grapvel_next = archer.grapple_vel;
						Vec2f check_forbidden_zone_step = grapvel_next/8 * i;
						Tile tile_step = map.getTile(check_forbidden_zone_step);

						if (tile_step.type == CMap::tile_castle_back_moss)
						{
							cancel_grapple = true;
						}
					}
				}
				if (tile.type == CMap::tile_castle_back_moss)
				{
					cancel_grapple = true;
				}
...
void ManageBow(CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars)
{
	...
	u8 charge_state = archer.charge_state;
	bool pressed_action2 = this.isKeyPressed(key_action2);
